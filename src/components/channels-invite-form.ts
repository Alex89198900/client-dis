...
class ChannelsInviteFormComponent extends Controller<ChannelsInviteFormView> {
  ...
  createInviteMessage(userId: string): FormData {
    const user = appStore.users.find((u) => u.id === userId);
    const username = user?.name || 'Unknown User';
    const formData = new FormData();
    formData.append('service', String(true));
    formData.append('channelId', this.channelId);
    formData.append('date', String(Date.now()));
    formData.append('message', `Invited ${username} to Channel`);
    formData.append('responsedToMessageId', String(null));
    return formData;
